  ...
  if ((new_field_flags & NOT_NULL_FLAG))
  {
    for (dict_foreign_t *foreign : user_table->foreign_set)
    {
      ...
      if ((foreign->type & foreign->UPDATE_CASCADE) &&
          foreign->check_fk_null_foreign(col_name))
        goto non_null_error;
...
non_null_error:
  ...
  else
  {
    ...
    {
      if ((foreign->type & foreign->UPDATE_CASCADE)
          && foreign->check_fk_null_referenced(col_name))
      {
        char display_name[FN_REFLEN];
	const int dblen=
          int(table_name_t(const_cast<char*>(
                             foreign->foreign_table_name)).dblen());
	sprintf(display_name, "%.*s.%s",
                dblen, foreign->foreign_table_name,
		foreign->foreign_table_name + dblen + 1);
        const char* fid = strchr(foreign->id, '/');
        fid= fid ? fid + 1 : foreign->id;
